...
export function StreakHistoryBar({ runs, longestStreak, startDate }: { runs: StreakRun[]; longestStreak: number; startDate: string }) {
  ...
  const totalDays = Math.max(1, Math.round((today.getTime() - start.getTime()) / 86400000));
  const W = 300, H = 12;
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <Svg width={W} height={H}>
        ...
        {runs.map((run, i) => {
          const offsetDays = Math.round((new Date(run.startDate+'T00:00:00').getTime() - start.getTime()) / 86400000);
